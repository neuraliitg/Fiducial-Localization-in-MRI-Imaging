...
sample = dicom.read_file("./Sampdata/SE5/IM39")
...
hull = cv2.convexHull(contours[cindex1],returnPoints = False)
defects = cv2.convexityDefects(contours[cindex1],hull)
defShape = defects.shape[0]
#print(defShape)
for i in range(defects.shape[0]):
    s,e,f,d = defects[i,0]
    start = tuple(conFor[s][0])
    end = tuple(conFor[e][0])
    far = tuple(conFor[f][0])
    cv2.line(rgbNorm,start,end,[0,255,0],2)
    cv2.line(rgbNorm,far,end,[0,255,0],2)
    cv2.line(rgbNorm,far,start,[0,255,0],2)
    cv2.circle(rgbNorm,far,2,[0,0,255],-1)
